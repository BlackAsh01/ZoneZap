...
/**
 * GET /api/users/by-id?userId=xxx - Returns id, name, email for a user.
 * Same logic as /api/users/[id]; static path for reliable deployment on Vercel.
 * Allowed when: requester is the user themselves, or requester is a guardian and userId is in their wards.
 */
export default async function handler(req, res) {
  ...
  if (!requestedId) return res.status(400).json({ error: 'userId query required' });
